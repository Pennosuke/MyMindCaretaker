...
const columns = [
  ...
  },
  {
    ...
    render: (nextRoute) =>
    <Link
      to={{pathname: `/user-data/${nextRoute.username}/user-program/${nextRoute.collection}/answer/${nextRoute.doc}`}}
    >
      <Button type="primary">
        ดูรายละเอียด
      </Button>
...
class ProgramDataPage extends Component {
  state = {
    data: [],
    mindfulnessData: [],
    dassData: [],
  ...
  componentDidMount() {
    document.title = "MMC : ประวัติการทำโปรแกรมฝึกปฏิบัติ"
  ...
  async fetch(username, collection) {
    ...
    const datasnapshot = await db.collection(collection).where('userName', '==', username).get()
...
export default ProgramDataPage;