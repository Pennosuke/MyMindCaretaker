...
class ExtraProgramAnswerPage extends Component {
  state = {
    userName: '',
    renderTimestamp: '',
    data: [],
  ...
  async fetch(username, collection, doc) {
    var newData = []
    var questionData = {}
    const programDataSnapshot = await db.collection('extraProgram').doc('archivement').collection('all').doc(collection).get()
    var programData = programDataSnapshot.data().contents
    ...
    const datasnapshot = await db.collection('extraProgram').doc('answerData').collection(collection).doc(doc).get()
    ...
    var renderTimestamp = this.renderDate(answerData.timestamp)
    this.setState({
              ...
              <Row>
                {this.state.isLoaded ?
                  (
                    <Col span={24}>
                      <Row>
                      <Col span={5} style={{ padding: '10px' }}>Username :</Col>
                      <Col span={19} style={{ padding: '10px' }}>{this.state.userName}</Col>
                      <Col span={5} style={{ padding: '10px' }}>วันที่ทำโปรแกรม :</Col>
                      <Col span={19} style={{ padding: '10px' }}>{this.state.renderTimestamp}</Col>
...
export default ExtraProgramAnswerPage;